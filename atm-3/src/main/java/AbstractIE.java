...
public abstract class AbstractIE {
		...
		for (Object property : getFactProperties(fact)) {
			if (propertyRequiresInputFromUser(property)) {
				try {
					...
					setFactPropertyValue(fact, property, ctor.newInstance(s));
				} catch (Exception ex) {
					ex.printStackTrace();
				}
	...
	protected abstract void setFactPropertyValue(Object fact, Object property, Object value);
	...
	private boolean checkRuleConditions(RuleOperator operator, List<Object> conditions) {
		boolean conditionMet = RuleOperator.AND.equals(operator);
	...
	protected abstract Object getFactPropertyValue(Object fact, Object property);
		...
		for (Object fact : facts) {
			...
			for (int i = 0; i < getNrOfFactProperties(fact); i++) {
				...
				if (isFactPropertyMandatory(slot)) {
					...
					factExists = factExists && getFactPropertyValue(fact, slot).equals(getFactPropertyValue(factToCheck, slotToCheck));
				}
			}
